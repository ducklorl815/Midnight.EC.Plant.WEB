...
public class PlantProfileService
{
    ...
    public async Task<PlantProfileDto?> GetByPlantIdAsync(Guid plantId, CancellationToken cancellationToken = default)
    {
        var profile = await _profileRepository.GetByPlantIdAsync(plantId, cancellationToken);
        return profile?.ToDto();
    }

    public async Task<PlantProfileDto> SaveAsync(Guid plantId, PlantProfileDto model, CancellationToken cancellationToken = default)
    {
        ...
        if (profile == null)
        {
            profile = new Midnight.EC.Plant.WEB.Models.Models.PlantProfileModel
            {
                PlantId = plant.Id,
                CreatedAt = now
            };
    ...
    public async Task SetAiEnvironmentAdviceAsync(Guid plantId, string? advice, CancellationToken cancellationToken = default)
    {
        var plant = await _plantRepository.GetByIdAsync(plantId, cancellationToken)
            ?? throw new InvalidOperationException("找不到植物。");
        ...
        if (profile == null)
        {
            profile = new Midnight.EC.Plant.WEB.Models.Models.PlantProfileModel
            {
                ...
                CreatedAt = now,
                UpdatedAt = now,
                AiEnvironmentAdvice = advice
            };
            await _profileRepository.InsertAsync(profile, cancellationToken);
        }
        else
        {
            profile.AiEnvironmentAdvice = advice;
            profile.ModifyDate = now;
            await _profileRepository.UpdateAsync(profile, cancellationToken);
        }
    }

    private static void MapProfile(Midnight.EC.Plant.WEB.Models.Models.PlantProfileModel profile, PlantProfileDto model, DateTime now)
    {
        ...
        profile.SaucerState = model.SaucerState;
        ...
        // 保留既有 AI 適配建議；僅由 SetAiEnvironmentAdviceAsync 覆寫
        if (model.AiEnvironmentAdvice != null)
        {
            profile.AiEnvironmentAdvice = model.AiEnvironmentAdvice;
        }
        profile.ModifyDate = now;
    }
}
